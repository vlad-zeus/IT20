...
$(document).ready(function () {
    ...
    $(add_number = function () {
        $('.square').each(function (index, element) {
            element.innerHTML = '<span style="font-size: 40px; color: white">' + (index + 1) + '</span>';
        }).css({
            "display": "flex",
    ...
    $(replace_square_color_3_to_red_khaki_yellow = function (color1, color2, color3) {
        $('.square').each(function (index, element) {
            if ((index + 1) % 3 === 0) {
                $(element).css({
    ...
    setTimeout(replace_square_color_to_purple, 4000, '#69069a', '100px', '100px');
    setTimeout(add_number, 6000);
    setTimeout(replace_square_color_3_to_red_khaki_yellow, 8000, "#FA0000FF", "#70744E", "#FFD400FF");
