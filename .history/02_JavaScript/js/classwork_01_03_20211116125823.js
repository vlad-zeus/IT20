...
function replace_style_square_to_yellow_red(color1, color2, color3) {
    ...
    for (let i = 0; i < elements.length; i++) {
        if (k % 15 === 0) {
            elements[
                i
            ].style.background = `linear-gradient(to right bottom, transparent 50%, ${color3} 50%) left / 50% 100% no-repeat, linear-gradient(to left bottom, transparent 50%, ${color3} 50%) right / 50% 100% no-repeat`;
        }
        k++;
    }
