...
first_elem.addEventListener("mouseover", (event) =>
  console.log(
    `Высота: ${first_elem.clientHeight}, ширина: ${first_elem.clientWidth}`
  )
);
first_elem.addEventListener("click", (event) =>
  alert("Цвет прямоугольнка - голубой")
);

let second_elem = document.querySelector(".yellow");
second_elem.addEventListener("mouseover", (event) =>
  console.log(
    `Высота: ${second_elem.clientHeight}, ширина: ${second_elem.clientWidth}`
  )
);
second_elem.addEventListener("click", (event) =>
  alert("Цвет прямоугольнка - желтый")
);
